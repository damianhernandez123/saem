...
?>
<script>
    var idiciclo = "<?php echo $_GET['idiciclo']; ?>";
</script>
<div class="card">
    <div class="card-body">
                ...
                <div class="d-flex">
                    <div class="p-2 mr-auto">
                        <div class="page-header-title">
                            ...
                            <div class="d-inline">
                                <h4>Actualizar información del ciclo Escolar</h4>
...
<script>
    ...
    $(document).ready(function () {
        $.ajax({
            ...
            data: "action=getCicloByID&idiciclo=" + idiciclo,
            success: function (text) {
                var servicio = text.data[0];
                $("#ciclo").val(servicio.ciclo);
                $("#abrev").val(servicio.abrev);
                $("#inicio").val(servicio.inicio);
                $("#termino").val(servicio.termino);
                $("#limite_inscipcion").val(servicio.limite_inscipcion);
                $("#idiciclo").val(servicio.idiciclo);
            }
        });
    });
    ...
    function submitForm() {
        ...
        if (r) {
            ...
            $.ajax({
                type: "POST",
                url: "dataConect/API.php",
                data: "action=updateCiclo&" + dataString,
                success: function (text) {
                    if (text == "success") {
                        formSuccess();
                        swalert("Exito!", 'Ciclo actualizado correctamente', 'success');
                    } else {
                        formError();
